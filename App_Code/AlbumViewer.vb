...
Public Class AlbumViewer
    ...
    Private Function GetPhotos(ByVal subId As String, ByVal MainName As String, ByVal SubName As String) As String
        ...
        For sp = 0 To count
            Try
                ...
                HtmlString &= " <a href='Farms/Albums/" & MainName.Trim & "/" & SubName.Trim & "/" & url & "' data-lightbox='" & SubName.Trim & "' data-title='" & Name & "'><Image  runat='server'  src='Farms/Albums/" & MainName.Trim & "/" & SubName.Trim & "/" & url & "' style='height: 225px; width: 225px'/></a>"
    ...
    Private Function GetDisplayGallerySub(mainId As String) As DataSet
        ...
        cmd.Parameters.AddWithValue("@id", thisid.Trim)
        Dim str As String = ""
        str &= " SELECT * FROM GallerySub WHERE (mainId = @mainId) "
        If thisalbumType = "Farm" Then
            str &= " AND FarmId = @id"
        End If
        cmd.CommandText = str
    ...
    Private Function GetDisplayGalleryPhotos(ByVal subId As String) As DataSet
        ...
        cmd.Parameters.AddWithValue("@id", thisid.Trim)
        Dim str As String = ""
        str &= " SELECT photoId, subId, Url, Name FROM GalleryPhotos WHERE (subId = @subId) "
        If thisalbumType = "Farm" Then
            str &= " AND FarmId = @id"
        End If
        cmd.CommandText = str
