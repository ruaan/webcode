...
Partial Class Farms_frmPackagesManager
...
#Region "Events"
    ...
    End Sub
    Protected Sub ddlAnimals_DataBound(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlAnimals.DataBound
        ddlAnimals.Items.Insert(0, New ListItem("-Select-"))
    End Sub
    Protected Sub btnAdd_Click(sender As Object, e As EventArgs) Handles btnAdd.Click
        If ddlAnimals.SelectedValue.Trim <> "-Select-" Then
            SaveAnimals()
            PopulateAnimals()
            ddlAnimals.SelectedValue = "-Select-"
        End If
    End Sub
    Protected Sub grdAnimals_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles grdAnimals.RowCommand
        If e.CommandName = "Remove" Then
            RemoveAnimal(grdAnimals.Rows(e.CommandArgument).Cells(0).Text.Trim)
        End If
    End Sub
...
#Region "Methods"
    Private Sub PopulatePackage()
        ...
        Dim str As String = ""
        str = "  SELECT packageId, Package, farmId, cost, LargeImage, Image1, Image2, Image3, Description, FromDate, ToDate FROM tbl_Farm_Packages WHERE (packageId = @packageId) "
    ...
    Private Sub PopulateFarms()
        ...
        Dim str As String = ""
        str &= " SELECT id, FarmName FROM tbl_Farm ORDER BY FarmName "
    ...
    End Sub
    Private Sub PopulateAnimalsDropdown()
        Dim ds As New DataSet()
        Dim sqlconnection2 As New SqlClient.SqlConnection
        sqlconnection2.ConnectionString = Web.Configuration.WebConfigurationManager.ConnectionStrings.Item("MainConnection").ToString
        Dim cmd As New SqlClient.SqlCommand
        ...
        Dim str As String = ""
        str &= " SELECT id, Name FROM tbl_Animals ORDER BY Name "
        ...
        Try
            ...
            ddlAnimals.DataSource = ds.Tables(0)
            ddlAnimals.DataBind()
        Catch ex As Exception
    ...
    Private Sub SaveAnimals()
        ...
        SavePackage()
        cmd.Parameters.AddWithValue("@packageId", lblPackageId.Text.Trim)
        cmd.Parameters.AddWithValue("@animalId", ddlAnimals.SelectedValue.Trim)

        Dim str As String = ""

        str &= " BEGIN TRAN"
        str &= " IF EXISTS (SELECT * FROM tbl_Farm_Package_Animals WHERE (packageId = @packageId) AND (animalId = @animalId)) "
        str &= " BEGIN"
        str &= " UPDATE  tbl_Farm_Package_Animals SET packageId = @packageId, animalId = @animalId WHERE (packageId = @packageId) AND (animalId = @animalId) "
        str &= " END "
        str &= " ELSE "
        str &= " BEGIN"
        str &= " INSERT INTO tbl_Farm_Package_Animals (packageId, animalId) "
        str &= " VALUES (@packageId, @animalId) "
        str &= " END  "
        str &= " COMMIT TRAN"
    ...
    Private Sub PopulateAnimals()
        ...
        Dim str As String = ""
        str &= " SELECT tbl_Farm_Package_Animals.packageId, tbl_Farm_Package_Animals.animalId, tbl_Animals.Name "
        str &= " FROM tbl_Farm_Package_Animals INNER JOIN tbl_Animals ON tbl_Farm_Package_Animals.animalId = tbl_Animals.id "
    ...
    Private Sub SavePackage()
        ...
        If lblPackageId.Text.Trim <> "" Then
            str &= " UPDATE  tbl_Farm_Packages SET Package = @Package, farmId = @farmId, cost = @cost, Description = @Description, FromDate = @FromDate, ToDate = @ToDate WHERE (packageId = @packageId) "
        Else
            str &= " INSERT INTO tbl_Farm_Packages (Package, farmId, cost, Description, FromDate, ToDate) VALUES (@Package, @farmId, @cost, @Description, @FromDate, @ToDate) "
        ...
        End Try
    End Sub
    Private Sub RemoveAnimal(ByVal animalsId As String)
        Dim myconnection As New SqlClient.SqlConnection
        myconnection.ConnectionString = Web.Configuration.WebConfigurationManager.ConnectionStrings.Item("MainConnection").ToString
        Dim cmd As New SqlClient.SqlCommand

        cmd.Parameters.AddWithValue("@animalId", animalsId.Trim)
        cmd.Parameters.AddWithValue("@packageId", lblPackageId.Text.Trim)

        Dim str As String = ""

        str &= " DELETE FROM tbl_Farm_Package_Animals WHERE animalId = @animalId AND packageId = @packageId "
    ...
    Private Sub SaveIncluded()
        ...
        str &= " BEGIN TRAN"
        str &= " IF EXISTS (SELECT * FROM tbl_Farm_Package_Included WHERE (packageId = @packageId) AND (Description = @Description)) "
        str &= " BEGIN"
        str &= " UPDATE  tbl_Farm_Package_Included SET packageId = @packageId, Description = @Description WHERE (packageId = @packageId) AND (Description = @Description) "
        str &= " END "
        str &= " ELSE "
        str &= " BEGIN"
        str &= " INSERT INTO tbl_Farm_Package_Included (packageId, Description) "
    ...
    Private Sub RemoveIncluded(ByVal includedId As String)
        ...
        str &= " DELETE FROM tbl_Farm_Package_Included WHERE includedId = @includedId AND packageId = @packageId "
    ...
    Private Sub SaveExcluded()
        ...
        str &= " BEGIN TRAN"
        str &= " IF EXISTS (SELECT * FROM tbl_Farm_Package_Excluded WHERE (packageId = @packageId) AND (Description = @Description)) "
        str &= " BEGIN"
        str &= " UPDATE  tbl_Farm_Package_Excluded SET packageId = @packageId, Description = @Description WHERE (packageId = @packageId) AND (Description = @Description) "
        str &= " END "
        str &= " ELSE "
        str &= " BEGIN"
        str &= " INSERT INTO tbl_Farm_Package_Excluded (packageId, Description) "
        str &= " VALUES (@packageId, @Description) "
        str &= " END  "
        str &= " COMMIT TRAN"
    ...
    Private Sub RemoveExcluded(ByVal excludedId As String)
        ...
        str &= " DELETE FROM tbl_Farm_Package_Excluded WHERE excludedId = @excludedId AND packageId = @packageId "
    ...
    Private Sub UpdatePicture(ByVal FileName As String)
        ...
        If ddlPicType.SelectedValue.Trim = "Large" Then
            ...
            str = " UPDATE tbl_Farm_Packages SET LargeImage = @LargeImage WHERE (packageId = @packageId) "
        ElseIf ddlPicType.SelectedValue.Trim = "Below first" Then
            cmd.Parameters.AddWithValue("@Image1", FileName.Trim)
            str = " UPDATE tbl_Farm_Packages SET Image1 = @Image1 WHERE (packageId = @packageId) "
        ElseIf ddlPicType.SelectedValue.Trim = "Below second" Then
            cmd.Parameters.AddWithValue("@Image2", FileName.Trim)
            str = " UPDATE tbl_Farm_Packages SET Image2 = @Image2 WHERE (packageId = @packageId) "
        ElseIf ddlPicType.SelectedValue.Trim = "Below third" Then
            cmd.Parameters.AddWithValue("@Image3", FileName.Trim)
            str = " UPDATE tbl_Farm_Packages SET Image3 = @Image3 WHERE (packageId = @packageId) "
        End If
