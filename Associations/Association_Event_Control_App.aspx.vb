...
Imports System.IO
Partial Class Associations_Association_Event_Control_App
    ...
    Private Function GetEvents() As String
        ...
        For sp = 0 To count
            ...
            If thisAttending(0) <> "0" Or thisAttending(1) <> "0" Then
                htmlString &= " <div class='auto-style6' style='background-color: #666666; color: #A2A3A3; display: inline-block; padding-right:  5px;  padding-left:  5px; cursor: pointer' onclick=""window.open('Association_Event_Control_Attendance_App.aspx?id=" & dsTerms.Tables(0).Rows(sp).Item("event_id").ToString.Trim & "', '_blank')"">"
                htmlString &= "                         View"
                htmlString &= " </div>"
            End If
            htmlString &= "         </div>"
            ...
            If System.IO.File.Exists(SavePath & dsTerms.Tables(0).Rows(sp).Item("event_id").ToString.Trim & "QRcode.jpg") Then
                htmlString &= "         <div style='padding: 10px; display: inline-block; width:100%'>"
                htmlString &= "        <img alt='' src='QrCodes/" & dsTerms.Tables(0).Rows(sp).Item("event_id").ToString.Trim & "QrCode.jpg' class='qrImage_Style'/> "
                htmlString &= "         </div>"
            End If
